...
public sealed class FreeAgencyV2Service
{
    ...
    private static FreeAgentMotivationProfile BuildMotivationProfile(FreeAgent agent, int index)
    {
        var motivations = new[]
        {
            ...
            new FreeAgentMotivationScore(FreeAgentMotivation.Winning, agent.FinalContractPreference?.PrefersContender == true ? 88 : 35 + index % 45, agent.FinalContractPreference?.PrefersContender == true ? "Veteran is prioritizing one credible championship opportunity." : "Winning culture is a factor."),
            new FreeAgentMotivationScore(FreeAgentMotivation.Development, agent.Age <= 24 ? 88 : 42, "Development path matters more for younger players."),
